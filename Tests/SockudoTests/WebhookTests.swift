...
final class WebhookTests: XCTestCase {
    ...
    func testVerifyChannelOccupiedWebhookSucceeds() {
        ...
        Self.sockudo.verifyWebhook(request: TestObjects.Webhooks.channelOccupiedWebhookRequest) { result in
    ...
    func testVerifyChannelVacatedWebhookSucceeds() {
        let expectation = XCTestExpectation(function: #function)
        Self.sockudo.verifyWebhook(request: TestObjects.Webhooks.channelVacatedWebhookRequest) { result in
            self.verifyAPIResultSuccess(result, expectation: expectation) { webhook in
                XCTAssertEqual(webhook.createdAt, Date(timeIntervalSince1970: 1619602993))
                XCTAssertEqual(webhook.events.count, 1)
    ...
    func testVerifyMemberRemovedWebhookSucceeds() {
        let expectation = XCTestExpectation(function: #function)
        Self.sockudo.verifyWebhook(request: TestObjects.Webhooks.memberRemovedWebhookRequest) { result in
            self.verifyAPIResultSuccess(result, expectation: expectation) { webhook in
                XCTAssertEqual(webhook.createdAt, Date(timeIntervalSince1970: 1619602993))
                XCTAssertEqual(webhook.events.count, 1)
    ...
    func testMissingSockudoKeyHeaderWebhookFails() {
        let expectation = XCTestExpectation(function: #function)
        Self.sockudo.verifyWebhook(request: TestObjects.Webhooks.missingKeyHeaderWebhookRequest) { result in
            let expectedError = SockudoError.internalError(WebhookService.Error.xSockudoKeyHeaderMissingOrInvalid)
    ...
    func testInvalidSockudoKeyHeaderWebhookFails() {
        let expectation = XCTestExpectation(function: #function)
        Self.sockudo.verifyWebhook(request: TestObjects.Webhooks.invalidKeyHeaderWebhookRequest) { result in
            let expectedError = SockudoError.internalError(WebhookService.Error.xSockudoKeyHeaderMissingOrInvalid)
            self.verifyAPIResultFailure(result, expectation: expectation, expectedError: expectedError)
        }
        wait(for: [expectation], timeout: 10.0)
    ...
    func testMissingSockudoSignatureHeaderWebhookFails() {
        let expectation = XCTestExpectation(function: #function)
        Self.sockudo.verifyWebhook(request: TestObjects.Webhooks.missingSignatureHeaderWebhookRequest) { result in
            let expectedError = SockudoError.internalError(WebhookService.Error.xSockudoSignatureHeaderMissingOrInvalid)
            self.verifyAPIResultFailure(result, expectation: expectation, expectedError: expectedError)
        }
        wait(for: [expectation], timeout: 10.0)
    }
    ...
    func testInvalidSockudoSignatureHeaderWebhookFails() {
        let expectation = XCTestExpectation(function: #function)
        Self.sockudo.verifyWebhook(request: TestObjects.Webhooks.invalidSignatureHeaderWebhookRequest) { result in
            let expectedError = SockudoError.internalError(WebhookService.Error.xSockudoSignatureHeaderMissingOrInvalid)
            self.verifyAPIResultFailure(result, expectation: expectation, expectedError: expectedError)
        }
        wait(for: [expectation], timeout: 10.0)
    ...
    func testMissingBodyDataWebhookFails() {
        let expectation = XCTestExpectation(function: #function)
        Self.sockudo.verifyWebhook(request: TestObjects.Webhooks.missingBodyDataWebhookRequest) { result in
