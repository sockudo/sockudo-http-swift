...
struct TestObjects {
    ...
    struct Client {
        ...
        static let shared = Sockudo(options: try! SockudoClientOptions(appId: testAppId,
                                                                     key: testKey,
                                                                     secret: testSecret,
    ...
    struct ClientOptions {
        ...
        static let withCluster = try! SockudoClientOptions(appId: testAppId,
                                                          key: testKey,
                                                          secret: testSecret,
                                                          encryptionMasterKey: testEncryptionMasterKey,
                                                          cluster: testCluster)

        static let withCustomHost = try! SockudoClientOptions(appId: testAppId,
                                                             key: testKey,
                                                             secret: testSecret,
                                                             encryptionMasterKey: testEncryptionMasterKey,
                                                             host: testCustomHost)

        static let withCustomPort = try! SockudoClientOptions(appId: testAppId,
                                                             key: testKey,
                                                             secret: testSecret,
                                                             encryptionMasterKey: testEncryptionMasterKey,
                                                             host: testCustomHost,
        ...
        static let withInvalidMasterKey = try! SockudoClientOptions(appId: testAppId,
                                                                   key: testKey,
                                                                   secret: testSecret,
                                                                   encryptionMasterKey: invalidEncryptionMasterKey,
                                                                   cluster: testCluster)

        static let withInvalidPrefixHost = try! SockudoClientOptions(appId: testAppId,
                                                                    key: testKey,
                                                                    secret: testSecret,
                                                                    encryptionMasterKey: invalidEncryptionMasterKey,
                                                                    host: invalidPrefixCustomHost)

        static let withInvalidSuffixHost = try! SockudoClientOptions(appId: testAppId,
                                                                    key: testKey,
                                                                    secret: testSecret,
                                                                    encryptionMasterKey: invalidEncryptionMasterKey,
    ...
    struct Events {
        ...
        static let withIdempotencyKey = try! Event(name: "my-event",
                                                    data: Self.eventData,
                                                    channel: TestObjects.Channels.public,
                                                    idempotencyKey: "test-idempotency-key-123")

        static let encryptedWithIdempotencyKey = try! Event(name: "my-event",
                                                             data: Self.eventData,
                                                             channel: TestObjects.Channels.encrypted,
    ...
    struct Webhooks {
        ...
        private static func webhookRequest(for webhook: Webhook? = nil,
                                           ...
                                           sockudoSignatureHeaderValue: String? = Client.testSecret) -> URLRequest {
